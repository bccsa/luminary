#!/bin/sh
# Adapted from https://github.com/cobot/couchdb-action/blob/master/entrypoint.sh

echo "Starting Docker..."
sh -c "docker run -d -p 5984:5984 -p 5986:5986 --tmpfs /ram_disk couchdb:3.3.3"

# CouchDB container name
export NAME=`docker ps --format "{{.Names}}" --last 1`

docker exec $NAME sh -c 'mkdir -p /opt/couchdb/etc/local.d && echo "[couchdb]\ndatabase_dir = /ram_disk\nview_index_dir = /ram_disk\ndelayed_commits = true\n[httpd]\nsocket_options = [{nodelay, true}]" >> /opt/couchdb/etc/local.d/01-github-action-custom.ini'
