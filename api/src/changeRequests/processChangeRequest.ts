...
export async function processChangeRequest(
    ...
    if (
        changeRequest.doc.type === DocType.Language &&
        changeRequest.doc.deleteReq &&
        (changeRequest.doc as LanguageDto).default
    ) {
    ...
    if (!doc.deleteReq) {
        ...
        if (doc.type == DocType.Post || doc.type == DocType.Tag) {
            ...
            if ((doc as PostDto).imageData) {
                const prevImageData =
                    prevDoc.docs.length > 0 ? prevDoc.docs[0].imageData : undefined;
            ...
            const addedTags = (doc as PostDto | TagDto).tags.filter(
                (tag) => !prevTags.includes(tag),
            );
            const removedTags = prevTags.filter(
                (tag) => !(doc as PostDto | TagDto).tags.includes(tag),
            );
    ...
    // Cascade delete for Post and Tag documents
