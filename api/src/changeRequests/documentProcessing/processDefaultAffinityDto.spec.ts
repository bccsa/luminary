    ...
    describe("config", () => {
        ...
        it("clamps out-of-range fields and falls back per-field to defaults when malformed", () => {
            const doc = makeDoc(
                ...
                DEFAULT_AFFINITY_ID,
                {
                    tierHalfLifeDays: { core: 999999, strong: 60, established: 25, unprotected: -5 },
                    tierWeight: { core: 5, strong: 0.6, established: 0.3, unprotected: 0.3 },
                    hitWeight: 5,
                    minScore: 0,
                    maxTags: 5000,
                    depthScale: 0,
            ...
            processDefaultAffinityDto(doc);
            expect(doc.config?.tierHalfLifeDays).toEqual({
                core: 3650,
                strong: 60,
                established: 25,
                unprotected: 1,
            });
            expect(doc.config?.tierWeight.core).toBe(2);
        ...
        it("passes through a well-formed config unchanged", () => {
            const custom: AffinityConfigDto = {
                tierHalfLifeDays: { core: 100, strong: 50, established: 20, unprotected: 40 },
                tierWeight: { core: 0.9, strong: 0.5, established: 0.25, unprotected: 0.25 },
                hitWeight: 0.05,
                minScore: 0.02,
                maxTags: 40,
                depthScale: 15,
