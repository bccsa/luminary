...
describe("FindService", () => {
    ...
    it("performs in-memory permission filtering when no type specified", async () => {
        const access = { [DocType.Group]: ["g1"], [DocType.Content]: ["g1"] } as any;
        ...
        dbService.executeFindQuery.mockResolvedValueOnce({
            docs: [
                ...
                { _id: "g2", type: DocType.Group },
                {
                    ...
                    parentType: DocType.Content,
                    memberOf: ["g1", "x"],
                },
                { _id: "c2", type: DocType.Content, memberOf: ["g2"] },
            ],
        });

        const result = await service.find(query, "token");

        // Should keep group g1 and content c1 only
        expect(result.docs.map((d: any) => d._id).sort()).toEqual(["c1", "g1"]);
    });
    ...
    it("builds $or with parentType-specific memberOf for Content (non in-memory path)", async () => {
        // Provide groups for post vs tag; content is the query type, so non in-memory path
        const access = {
        ...
        const branchPost = sel.$or[0];
        const branchTag = sel.$or[1];

        expect(branchPost).toEqual(
            expect.objectContaining({
                type: DocType.Content,
    ...
    it("builds $or correctly when multiple types ($in) are specified", async () => {
        ...
        // Expect branches for:
        // - Post type
        // - Content with parentType Post
        // - Content with parentType Tag
        ...
        expectedBranches.forEach((branch) => {
            expect(sel.$or).toEqual(expect.arrayContaining([branch]));
        });
    });
});
