...
`AclPermission` includes **`CmsView`** (GitHub #160, ADR 0013) alongside View/Edit/Delete/Assign/Translate/Publish. `View` gates app/public reads (published content only); `CmsView` gates CMS-scoped reads/sync (drafts/scheduled/expired). The permission model is generic over the enum, so a `cms`-flagged request just swaps the permission passed to `accessMapToGroups` (`cms ? CmsView : View`) at the `/query`, `/fts`, and Socket.io call sites; no per-doctype special-casing.
...
Two room sets per group (GitHub #160, ADR 0013): **base `${docType}-${groupId}`** (app, joined via `View`) and **`${docType}-${groupId}-cms`** (CMS, joined via `CmsView`). The connection declares its mode in the `clientConfigReq` handshake (`cms: true | false` → `socket.data.cms`) because the AccessMap is per-user and can't distinguish an app from a CMS connection; a CMS connection joins **only** `-cms` rooms. (`clientConfigReq` was formerly `joinSocketGroups`, kept as a deprecated alias for older clients — ADR 0005; the whole Socket.io live-update transport is slated to migrate to SSE.) `deleteCmd-${groupId}` rooms are shared (un-suffixed) by both modes. On the handshake the server emits a `clientConfig` (max upload sizes + accessMap) then joins the rooms its accessMap grants for that mode. `DbService` `update` events fan out by resolving the doc's groups (for Content: via its parent's `memberOf`; for Change docs: via the embedded `changes` payload): **`-cms` rooms get the full doc for every status**; **base rooms get** published-and-live Content + all non-Content full, **expired** Content as a `stripExpiredContent()` stub (`util/stripExpiredContent.ts` — preserves the #433 prune-not-orphan behaviour), and **draft** Content not at all (the app is evicted via the app-only `DeleteReason.StatusChange` DeleteCmd). Auth happens in a Socket.io middleware before the connection is accepted.
...
- `POST /fts` (`ftsSearch.controller.ts` + `ftsSearch.service.ts`) — server-side full-text search complementing offline FTS (ADR 0010). Reproduces the client's trigram + BM25 ranking against the full corpus using two CouchDB views (`fts-trigram-index` splits each `fts` entry to one row per trigram with doc-level filter metadata in the value; `fts-corpus-stats` serves `_stats` for avg doc length). Permission (`cms ? CmsView : View`, GitHub #160) + (non-`cms`) published/scheduled/expired/language filtering is done in JS from the embedded view-row metadata before the top-K cap, then the top-K docs are fetched (by-key `_all_docs`) for full BM25 + word-match parity (`util/ftsScoring.ts`). (No expired-stripping needed here: the non-`cms` path already drops expired/draft Content, so `/fts` never returns one to a non-CMS caller.) Returns ranked page bodies **trimmed of `fts`/`ftsTokenCount` — display-only, clients must not persist them**. The same endpoint also serves a **strict aux path** for non-Content doctypes (`User`, `Redirect`): when `types` is a single such doctype, `searchAux` reads that doctype's own trigram view (`fts-trigram-index-user` / `fts-trigram-index-redirect`, which emit a named metadata object instead of the Content positional tuple) and does substring-AND on the searchable fields + a field sort + permission/`groups` filtering — no BM25, no corpus stats (`AUX_FTS_CONFIG` in `ftsSearch.service.ts`). The `fts` index for these doctypes is computed in `processUserDto`/`processRedirectDto` (strict-only field configs `USER_FTS_FIELDS`/`REDIRECT_FTS_FIELDS`, no client mirror) and backfilled by schema upgrade v18.
- `GET /storage/storagestatus` (`storageStatus.controller.ts`) — bucket connectivity probe; requires `CmsView` on the `Storage` doc (the CMS bucket overview is its only consumer).
- `GET /sidecar` (`sidecar.controller.ts`) — the only read path for `Sidecar` docs, which are never replicated (ADR 0019); it serves the masked HLS decryption key. Two independently-bucketed rate limiters (`ratelimit/sidecarRateLimiter.service.ts`) guard it and, unlike the query limiter, **both default ON**: `SIDECAR_RATE_LIMIT_READ_*` bounds successful key fetches, `SIDECAR_RATE_LIMIT_PROBE_*` bounds repeated 403/404s at a lower ceiling. A blocked caller gets 429, which `shared`'s http layer swallows — so the app and CMS players surface it as media needing a key rather than as a rate limit. Check these before diagnosing a decryption fault.
