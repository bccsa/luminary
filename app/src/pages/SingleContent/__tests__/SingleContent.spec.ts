...
describe("SingleContent", () => {
    ...
    it("does not show 404 page while content is loading", async () => {
        // Start with content in the database
        const wrapper = mount(SingleContent, {
            props: {
                slug: mockEnglishContentDto.slug,
            },
        ...
        // Clear database to simulate network delay/loading state
        await db.docs.clear();

        // Trigger a route change by updating the slug prop
        // This simulates navigating to different content
        await wrapper.setProps({ slug: "some-other-slug" });

        // Give time for the component to react to the empty database
        await new Promise((resolve) => setTimeout(resolve, 100));

        // Re-add content to simulate loading completing
        await db.docs.bulkPut([
            mockEnglishContentDto,
            mockFrenchContentDto,
            mockLanguageDtoEng,
        ...
        // Update slug back to valid content
        ...
        // Verify 404 never appeared during the transition
