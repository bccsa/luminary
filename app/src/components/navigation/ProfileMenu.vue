...
import { appLanguageAsRef } from "@/globalConfigOld";
import {
    ...
    <div>
        <Menu as="div" class="relative z-50">
            <MenuButton name="profile-menu-btn" class="-m-1.5 flex items-center p-1.5">
                ...
                <img class="h-8 min-w-8 rounded-full bg-slate-50" :src="user?.picture"
                    v-if="isAuthenticated && user?.picture" alt="" />
                ...
                <span class="hidden lg:flex lg:items-center">
                    <span
                        ...
                        aria-hidden="true" v-if="isAuthenticated">{{ user?.name }}</span>
                    <span class="ml-2 text-sm font-semibold leading-6 text-zinc-900 dark:text-slate-50"
                        aria-hidden="true" v-else>Menu</span>
            ...
            <transition enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems name="menu-items"
                    ...
                    <button :class="[
                        ...
                        <component :is="item.icon" class="h-5 w-5 flex-shrink-0 text-zinc-500 dark:text-slate-300"
                            aria-hidden="true" />
                        <div class="flex flex-col text-nowrap leading-none">
                            {{ item.name }}
                            <span v-if="'language' in item && item.language"
                                class="mt-1 text-[12px] text-zinc-500 dark:text-white">{{ item.language }}</span>
                        </div>
                    </button>
                    </MenuItem>
                </MenuItems>
        ...
        <LDialog v-model:open="showLogoutDialog" :title="t('logout.modal.title')"
            :description="t('logout.modal.description')" :primaryAction="confirmLogout"
            :primaryButtonText="t('logout.modal.button_logout')" :secondaryAction="() => (showLogoutDialog = false)"
