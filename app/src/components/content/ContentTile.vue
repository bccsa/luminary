...
import { getMediaDuration, getMediaProgress } from "@/globalConfigOld";
import { ref } from "vue";
...
<template>
    <RouterLink :to="{ name: 'content', params: { slug: props.content.slug } }"
                    ...
                    <template #imageOverlay>
                        ...
                        <div v-if="titlePosition !== 'center'">
                            <div v-if="content.video"
                                ...
                                <PlayIcon class="relative h-8 w-8 text-black blur-sm lg:h-12 lg:w-12" />
                            </div>
                            <div v-if="content.video"
                                class="absolute inset-0 flex items-center justify-center rounded-lg">
                        ...
                        <div v-else
                            ...
                            <p class="absolute m-2 text-pretty text-center text-white dark:text-slate-200">
                                {{ content.title }}
                            </p>
                        </div>

                        <!-- Bottom overlay: progress bar + duration on same line -->
                        <div v-if="showProgress && content.video && hasProgress"
                            class="absolute bottom-2 left-0 right-0 z-10 mx-1 rounded-md bg-black/50 px-1">
                            <div class="flex h-4 w-full items-center gap-2">
                                <!-- Progress bar -->
                                <div class="relative h-2 flex-1 overflow-hidden rounded bg-zinc-600">
                                    <div class="absolute left-0 top-0 h-full bg-white"
                                        :style="{ width: `${media.progress}%` }"></div>
                                </div>
