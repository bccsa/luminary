...
<template>
    <Teleport to="body" :disabled="isTestEnviroment">
        <div v-if="open" @click="open = false">
            <div
                class="fixed inset-0 z-50 bg-zinc-800/75 transition-opacity dark:bg-slate-800/50"
                data-test="dialog-modal"
            ></div>
            <div class="fixed inset-0 z-50 flex items-center justify-center rounded-lg p-2">
                <div
                    class="relative transform overflow-hidden rounded-lg bg-white/90 px-4 pb-4 pt-5 text-left shadow-xl dark:bg-slate-700/85 sm:my-8 sm:w-full sm:max-w-lg sm:p-6"
                    @click.stop
                >
