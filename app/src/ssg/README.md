...
- **Two data tiers.** **Public** content is prerendered into the static HTML (+ a
  ...
  *never* prerendered — it syncs to the client at runtime. This folder only ever touches
...
+ default language to bound page weight. The render language also rides `initialState`.
...
content with `updatedTimeUtc` newer than the last seen → `keysForChangedDoc` →
...
| 2 | **No-flash hydration via shared's response cache** | Clean hydration *without* a bespoke snapshot. `useHybridQuery({cache:true})` already reads `hqcache:*` synchronously — so the prerender just primes it. |
...
| 5 | **Derive dependency keys generically from the query selector** | Rearranging layout / adding a page needs **zero** key edits — only a new *data facet* touches `facetKeys.ts`. Rejected: hardcoding keys per page. |
...
lists with the `hqcache` first-paint seed. `/explore` + `/watch` prerender. Sidebar uses
real anchors. i18n SSR renders strings in the right language.
...
- **Watcher: deletes** (`DeleteCmd` → remove `<slug>.html` + its manifest entry +
  regenerate co-listed pages) and **redirects** (`RedirectDto` → purge the source path;
  the runtime SPA handles the redirect).
...
- The client-entry rewrite (`main.ts` → `main.web.ts`) **must be a `pre`
  ...
  no-op). Symptom when wrong: the *prerender* uses `main.web.ts` (HTML looks right, JS-off
  works) but the *client* boots `main.ts` — the full native app, with its service worker,
...
- Never import `src/` modules into `vite.config.web.ts` (TS project-reference errors) —
  talk to the collector via `globalThis.__SSG_DEPS__`.
...
- Pure infra: `facetKeys.ts` / `dependencyCapture.ts` / `computeAffected.ts`. Watcher:
  `watch.ts` + `ssgNodeEnv.ts`.
