...
- **Block top-K.** Candidates pre-ranked by `Σ idf·tf` are exact-scored in fixed blocks of `FTS_TOP_K=150`, and a page fetches only the blocks it overlaps rather than a fixed 500 — each fetched doc drags its large `fts` array, so a smaller K means a much smaller doc fetch. Fixed blocks keep a result's position independent of the requested page; growing the set with `offset + limit` re-ranked earlier pages and repeated their results on later ones (#2079).
