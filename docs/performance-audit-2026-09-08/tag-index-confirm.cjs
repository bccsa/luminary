...
(async()=>{
 ...
 const rows=[];
 const pairs=[];
 for(const id of ['sync-content-tag-first']) {
 ...
 const output={startedAt:new Date().toISOString(),baseUrl:report.config.baseUrl,samples:15,warmup:1,identity:'anonymous',pairs,rows};
 for(const {id,original,candidate} of pairs) {
  ...
  fs.writeFileSync('perf-reports/query-sync/tag-index-confirmation.json',JSON.stringify(output,null,2));
