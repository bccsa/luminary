...
| **5** ✅ | [02](02-sidecar-rest-endpoint.md) + [09](09-authentication.md) + [10](10-retrieval-by-parent-id.md) | `GET /sidecar` | **Done** — `sidecar.controller.ts` mirrors `encoderConfig.controller.ts`: `@UseGuards(AuthGuard)` only (reading A — anonymous callers allowed, gated by the ordinary `View` check), parent-doc permission check (never the sidecar's copied `memberOf`), `isParentAvailable` (added to `sidecar.service.ts`) refusing draft/scheduled/expired parents, `Cache-Control: no-store`, 400/403/404/409 per the doc-02 table. `sidecar.controller.spec.ts` covers the full status-code matrix, the availability-predicate branches, and the write→read round-trip. CouchDB-backed, user-run — **not yet executed against a live CouchDB in this session** (no reachable instance); lint + typecheck clean. **Deferred, not built:** rate limiting and audit logging (doc 02 leaves the concrete limits/mechanism as an implementation-time decision, not a spec) — the endpoint is otherwise complete but these should be picked up as a fast follow before wide exposure. |
