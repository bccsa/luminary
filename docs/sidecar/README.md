...
| **3** ✅ | [04](04-hls-key-as-sidecar.md) + [07](07-no-encryption-at-rest.md) | `processMedia` writes a masked sidecar; the crypto path is deleted | **Done** — commit `e26c5b62`. `processMedia` writes a masked sidecar via `upsertHlsKeySidecar`; a failed store now fails the change request instead of silently dropping the key. `CryptoDto`/`storeCryptoData` retained for S3 credentials; no schema upgrade (1878 unshipped). ADR 0018 records the "masked, not encrypted at rest" decision. CouchDB-backed specs user-run. |
| **4** ✅ | [03](03-lifecycle-and-deletion.md) | Deletion on parent delete and on key removal | **Done** — `deleteSidecarsForParent` in the `deleteReq` branch (warn-not-throw, matching `deleteImage`'s precedent); key-removal detection outside the `if (doc.media)` block so the "whole media object removed" case — the one a check inside `processMedia` could never see — is reachable. Unmocked `processPostTagDto.sidecar.spec.ts` covers all eight doc-03 cases, including the `hlsKey_id` field-drop regression. CouchDB-backed specs user-run. |
| **5** ✅ | [02](02-sidecar-rest-endpoint.md) + [09](09-authentication.md) + [10](10-retrieval-by-parent-id.md) | `GET /sidecar` | **Done** — `sidecar.controller.ts` mirrors `encoderConfig.controller.ts`: `@UseGuards(AuthGuard)` only (reading A — anonymous callers allowed, gated by the ordinary `View` check), parent-doc permission check (never the sidecar's copied `memberOf`), `isParentAvailable` (added to `sidecar.service.ts`) refusing draft/scheduled/expired parents, `Cache-Control: no-store`, 400/403/404/409 per the doc-02 table. `sidecar.controller.spec.ts` covers the full status-code matrix, the availability-predicate branches, the write→read round-trip, and (added as a fast follow) the two rate limiters below. CouchDB-backed, user-run — **not yet executed against a live CouchDB in this session** (no reachable instance); lint + typecheck clean. **Rate limiting: built** — `SidecarRateLimiterService` (`api/src/ratelimit/`) wires a `read` limiter (bounds successful fetches, default ON) and a `probe` limiter (bounds repeated 403/404s, default ON, lower ceiling) into the endpoint; concrete limits recorded in ADR 0018. **Still deferred:** audit logging (doc 02's open question, unchanged). |
