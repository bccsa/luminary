...
const storageKey = computed(() => `queryOptions_${props.docType}_${props.tagOrPostType}`);

onMounted(() => {
    const savedQueryOptions = sessionStorage.getItem(storageKey.value);
    if (savedQueryOptions) {
        queryOptions.value = JSON.parse(savedQueryOptions);
    }
});

watch(queryOptions, () => {
    sessionStorage.setItem(storageKey.value, JSON.stringify(queryOptions.value));
}, { deep: true });
