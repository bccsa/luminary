...
type Props = {
    groupQuery: ApiLiveQuery<GroupDto>;
};
...
<template>
    <tr>
        ...
        <!-- status -->
        <td class="whitespace-wrap py-2 pl-4 pr-3 text-sm font-medium text-zinc-900 sm:pl-6">
            <LBadge v-if="!isEdited" variant="info">test</LBadge>
            <LBadge v-if="isEdited && isEdited(group._id)" variant="info">Edited</LBadge>
            <LBadge v-if="isModified && isModified(group._id)" variant="warning"
                >Incoming edit</LBadge
            >
        </td>
