...
const audioSrc = computed(() => {
    // Existing uploaded media (MediaFileDto) - use fileUrl as provided (no baseUrl)
    if (props.mediaFile) {
        const url = props.mediaFile.fileUrl || "";
        return url || "";
    }

    // Pending upload (MediaUploadDataDto) - prefer provided fileUrl, otherwise create object URL for local file data
    if (props.mediaUploadData) {
        const anyData: any = props.mediaUploadData as any;
        if (anyData.fileUrl) {
            return anyData.fileUrl as string;
        }
