...
import {
    ...
    ChevronRightIcon,
    ChevronLeftIcon,
    GlobeEuropeAfricaIcon as FilledGlobeEuropeAfricaIcon,
    ArrowUturnRightIcon,
    CloudIcon as FilledCloudIcon,
    ShieldCheckIcon,
...
const navigation = computed(() => [
    ...
    },
    {
        ...
        activeIcon: FilledGlobeEuropeAfricaIcon,
...
<template>
    ...
    <aside
        data-test="sidebar"
        @scroll.stop
        class="fixed inset-y-0 left-0 z-50 flex h-screen w-72 flex-col border-r border-zinc-200 bg-zinc-100 transition-[transform,width] duration-200 ease-out dark:border-slate-700 dark:bg-slate-800 lg:relative lg:z-30 lg:translate-x-0"
        :class="[
    ...
    >
        ...
        <button
            type="button"
            class="absolute right-0 top-1/2 z-20 hidden h-8 w-8 -translate-y-1/2 translate-x-1/2 items-center justify-center rounded-full border border-zinc-400 text-zinc-800 shadow-md transition-colors hover:bg-zinc-500 dark:bg-slate-700 dark:hover:text-yellow-400 lg:flex"
        ...
        <nav class="flex-1 overflow-y-auto overflow-x-hidden px-2 py-2 scrollbar-hide">
            <ul role="list">
                <li v-for="item in navigation" :key="item.name">
                    ...
                    >
                        <component
                            :is="isActive ? item.activeIcon || item.icon : item.icon"
                            :class="navIconClass"
                            aria-hidden="true"
                        />
            ...
            <div class="mt-2 border-t border-zinc-200 pt-3">
                ...
                <RouterLink
                    ...
                    v-slot="{ isActive }"
                >
                    <FilledCog6ToothIcon v-if="isActive" :class="navIconClass" aria-hidden="true" />
                    <Cog6ToothIcon v-else :class="navIconClass" aria-hidden="true" />
