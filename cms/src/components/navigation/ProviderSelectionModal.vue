...
<template>
    <LModal v-model:isVisible="showProviderSelectionModal" heading="Sign in" :noDivider="true">
        <p class="shrink-0 text-sm text-gray-500">
            Select an authentication provider to continue
        </p>

        <div class="flex flex-col gap-3 min-h-0 flex-1 overflow-y-auto">
            <button
        ...
        <template #footer>
            <div class="flex justify-end">
                <LButton variant="secondary" @click="handleClose">Cancel</LButton>
            </div>
        </template>
    </LModal>
</template>
