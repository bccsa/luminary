...
- Searches use `db.docs.where("fts").between(trigram + ":", trigram + ";")` per trigram, parse TF, compute BM25 against `corpusStats` (stored under `luminaryInternals["corpusStats"]`). Per-trigram doc frequencies are read from the ones stored with each corpus-stats recompute; with none stored, searches fall back to counting the index per trigram.
