 ...
 * starting values were simulation-validated, but remain open to future tuning. The
 * boundaries themselves (top-3/top-5/top-10) are structural to the ranking algorithm
 * and stay fixed; only the per-tier magnitudes in {@link AffinityConfig} are tunable.
 */
...
type Tier = ReturnType<typeof tierForRank>;
...
export type AffinityConfig = {
    /** Simulation-validated starting half-lives, in days, by profile rank tier. */
    tierHalfLifeDays: Record<Tier, number>;
    /** Simulation-validated starting retrieval multipliers by profile rank tier. */
 ...
 * independent scales.
 */
export function tierWeightForRank(
    rank: number,
    config: AffinityConfig = DEFAULT_AFFINITY_CONFIG,
): number {
    return config.tierWeight[tierForRank(rank)];
}
...
export function decay(
    ...
    ranked.forEach(([tag, score], index) => {
        const halfLife = config.tierHalfLifeDays[tierForRank(index + 1)];
