...
describe("Database", async () => {
    ...
    describe("getSyncList validation", () => {
        const validEntry = {
            chunkType: "post",
            memberOf: ["g1"],
            blockStart: 5000,
        ...
        it("loads a valid syncList", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [{ ...validEntry }, { ...validEntry, chunkType: "tag" }];
            await db.setSyncList();
        ...
        it("resets syncList when an entry has MAX_SAFE_INTEGER blockStart", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [
                { ...validEntry },
            ...
            syncList.value = []; // clear in-memory
            await db.getSyncList();
            expect(syncList.value).toHaveLength(0);
        });

        it("resets syncList when an entry has NaN block values", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [{ ...validEntry, blockStart: NaN }];
            await db.setSyncList();
        ...
        it("resets syncList when an entry has inverted range", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [{ ...validEntry, blockStart: 1000, blockEnd: 5000 }];
            await db.setSyncList();
        ...
        it("resets syncList when an entry has eof with narrow range (merge regression)", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [
                { ...validEntry, blockStart: 5000, blockEnd: 4000, eof: true }, // 1000ms = syncTolerance
            ];
            await db.setSyncList();
        ...
        it("keeps valid eof entry with blockEnd=0", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [{ ...validEntry, blockStart: 5000, blockEnd: 0, eof: true }];
            await db.setSyncList();
        ...
        it("keeps valid eof:false entry with narrow range (still syncing)", async () => {
            const { syncList } = await import("../rest/sync2/state");
            syncList.value = [{ ...validEntry, blockStart: 5000, blockEnd: 4500, eof: false }];
            await db.setSyncList();
